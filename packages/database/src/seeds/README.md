...
deterministic natural-key ids (`trc-*-NNN`) plus `ON CONFLICT DO NOTHING`.
...
```bash
# Seed the TRC organization only
SEED_ORG_SEEDS=true DATABASE_URL=postgres://... pnpm db:seed --org=trc
...
### `trc` — Tanzania Railway Commission

| Area              | Count / detail                                                              |
| ----------------- | --------------------------------------------------------------------------- |
| Tenant            | `trc-tenant` — country=TZ, currency=TZS, tier=enterprise                     |
| Organization      | `trc-org` — "TRC Head Office"                                                |
| Roles             | `trc_admin`, `trc_estate_manager`, `trc_station_master`                      |
...
| District polygons | Simplified bounding shapes from `trc-districts.json`                         |
| Approval policies | `maintenance_cost`, `lease_exception`, `payment_flexibility` with 100k/500k TZS thresholds |
| Invite codes      | `TRC-ONBOARD-001`, `TRC-ONBOARD-002`, `TRC-STATION-MASTER` (on `tenants.settings`) |
...
The 4 TRC districts follow the inverted hierarchy (Districts > Regions):
...
Polygon coordinates in `trc-districts.json` are simplified bounding shapes
(not survey-grade boundaries). Replace with authoritative TRC-published
...
├── trc-districts.json    # simplified TRC district polygons
└── trc-seed.ts           # TRC organization seed
```
