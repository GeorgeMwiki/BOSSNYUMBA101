...
import { flagPSAClauses, PSA_CLAUSE_SPECS } from '../loi-psa/psa-clause-flagger.js';
...
import { recommendEndorsements } from '../insurance/title-endorsement-recommender.js';
import type {
  ComparableSale,
  DealSnapshot,
  LOIAxisRating,
...
describe('sample Dar es Salaam 50-unit recommendation', () => {
  it('prints the recommendation', () => {
    const triangulation = triangulateSales(comps, {
      maxMonthsAgo: 18,
    ...
    const psa = flagPSAClauses({
      clauses: PSA_CLAUSE_SPECS.map((spec) => ({
        key: spec.key,
        present: true,
        buyerFavorable: true,
      })),
    ...
    const endorsements = recommendEndorsements(altaCommitment, {
      hasZoningCompletedStructure: true,
    ...
    console.log('\n=== SAMPLE DAR ES SALAAM 50-UNIT ACQUISITION RECOMMENDATION ===\n');
    // eslint-disable-next-line no-console
    console.log(JSON.stringify(
      {
        dealId: rec.dealId,
        verdict: rec.verdict,
        composite: Number(rec.composite.toFixed(3)),
        confidence: Number(rec.confidence.toFixed(3)),
        pricing: rec.pricingRecommendation,
        criticalFindings: rec.findings
          .filter((f) => f.severity === 'critical' || f.severity === 'deal-killer').length,
        closingChecklistTop: rec.closingChecklist.slice(0, 5),
