...
const tree = buildOrgUnitTree({
  ...
  units: [
    unit('geita', null, 'bossnyumba/geita'),
    unit('geita-site-a', 'geita', 'bossnyumba/geita/site-a'),
    unit('mererani', null, 'bossnyumba/mererani'),
  ],
});
...
describe('resolveUserScope', () => {
  ...
  it('classifies single org_unit binding as org_unit scope and sets scope_path', () => {
    ...
    expect(scope.resolved_terminology.scope_path).toBe('bossnyumba/geita');
  });
  ...
  it('returns legacy_mode when the tenant has no org units', () => {
    const emptyTree = buildOrgUnitTree({ tenantId: 't-bossnyumba', units: [] });
    const scope = resolveUserScope({
      tenantId: 't-bossnyumba',
      bindings: [binding('b-root', 'tenant_root', null, 2)],
      tree: emptyTree,
