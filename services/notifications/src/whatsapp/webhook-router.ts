...
export interface WebhookRouterOptions {
   ...
   * (`x-bossnyumba-internal-secret`); dev/tests can pass
...
export function createWebhookRouter(options: WebhookRouterOptions): Router {
  ...
  //
  //   1. Outbound WhatsApp fan-out from BOSSNYUMBA's Meta account to
  //      arbitrary numbers (cost + reputation + abuse-of-trust).
  //   2. Phone-number-existence oracle via session GET (info disclosure).
  //
  // The middleware compares the `x-bossnyumba-internal-secret` header
  ...
  const requireAdminAuthMiddleware = (
    ...
    const provided = req.headers['x-bossnyumba-internal-secret'];
    const providedStr =
