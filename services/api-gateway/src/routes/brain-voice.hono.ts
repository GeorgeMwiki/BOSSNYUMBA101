 ...
 *
 * This file owns auth, the locale-driven persona instruction, the pure inbound
 * frame router, the session bridge, and the WS-upgrade transport seam. The
 * Gemini Live duplex client + the shared provider contracts live in the
 * sibling `./brain-voice/gemini-upstream` module (keeps each file <800 lines).
 *
 ...
 *     acknowledged (`acknowledged`, executed:false) — never executed.
 *   • AUDIO CODEC: 16 kHz mono PCM little-endian in; 24 kHz PCM out. Opus
 *     transcode + sample-rate negotiation are out of scope here.
 *
...
} from './brain-voice/gemini-upstream';
...
// Re-export the shared contracts so existing consumers can import them from the
...
// surfaces do. The pure interface stays stable so callers don't change.
// ───────────────────────────────────────────────────────────────────────────
    ...
    switch (decoded.action) {
      ...
      case 'tool_result':
        if (this.requireReady()) {
          const out = { name: decoded.name, output: decoded.output };
