/**
 * CRITICAL #4 — per-tenant allowlist guard for the NGGIS MCP server, with
...
import { createNggisServer } from '../src/index.js';
import { CallToolRequestSchema } from '@modelcontextprotocol/sdk/types.js';

function getCallHandler(serverObj: ReturnType<typeof createNggisServer>) {
...
const ARGS = { tenantId: 't-bravo', stateCode: 'LA', query: 'plot 42' };

async function call(
  srv: ReturnType<typeof createNggisServer>,
  args: Record<string, unknown>,
) {
  const handler = getCallHandler(srv);
  if (!handler) return null;
  return (await handler({
    params: { name: 'nggis.search_property', arguments: args },
    method: 'tools/call',
  })) as { isError?: boolean; content?: Array<{ text?: string }> };
}

describe('NGGIS MCP allowlist — fail-closed in every env', () => {
  it('rejects when allowlist set and tenant not in it', async () => {
    const res = await call(createNggisServer({ allowlist: ['t-alpha'] }), ARGS);
    if (!res) return;
  ...
  it('rejects in NON-production with no allowlist (no dev fail-open)', async () => {
    ...
    try {
      const res = await call(createNggisServer({}), ARGS);
      if (!res) return;
      expect(res.isError).toBe(true);
      expect(res.content?.[0]?.text).toMatch(/not in the per-tenant allowlist/);
    } finally {
