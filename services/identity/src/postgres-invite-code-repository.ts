...
export interface InviteCodeRepositoryClient {
  ...
  // `execute` runs the raw `SELECT … FOR UPDATE` row-lock inside `redeem`.
  // Required because the redeem path is the only consumer and it always
  // runs against a postgres-js Drizzle client/tx, both of which expose it.
