/**
 * Cross-portal listener (Estate Manager App) — Central Command Phase A C6.
 *
...
export function startCrossPortalListener(
  ...
  const scheduleReconnect = (): void => {
    ...
    const backoff = Math.min(
      maxBackoffMs,
      // eslint-disable-next-line no-restricted-syntax -- reason: jitter for exponential backoff delay, not an ID or secret
