# @bossnyumba/staff-mobile - Production Readiness
...
- TypeScript strict: passes (`pnpm -F @bossnyumba/staff-mobile typecheck`).
...
| --- | --- | --- |
| `expo-camera`, `expo-image-picker` (camera launch) | `src/media/usePhotoPicker.ts` (W-M-04 work-order completion, delivery photo capture) | Library picker works; `launchCameraAsync` throws -> screen shows error state |
| `expo-location` | `src/location/useLocation.ts` (inspection GPS, attendance fences) | Permission request returns denied -> "GPS unavailable" UX |
...
2. `app.json` `extra.apiGatewayUrl`: flipped to `http://localhost:4001` so Expo Go talks to the local api-gateway out of the box.
3. `eas.json` development profile: `EXPO_PUBLIC_API_GATEWAY_URL` flipped to `http://localhost:4001`.
4. `src/api/config.ts`: hard fallback updated to `:4001`; `CHAT_PREFIX` set to `/api/v1/ai/chat` so SSE actually hits the Master Brain route.
...
pnpm -F @bossnyumba/staff-mobile start
