...
export interface BnSegmentedOption<T extends string> {
  readonly value: T
  readonly label: string
}

export interface BnSegmentedProps<T extends string> {
  readonly value: T
  readonly options: ReadonlyArray<BnSegmentedOption<T>>
  readonly onChange: (next: T) => void
  readonly testID?: string
}

/**
 * BossNyumba segmented control — `bg-slate-800 rounded-full p-1` pill
 * with gold active segment, navy text. Used for role pickers,
 * filter tabs, period toggles.
 */
export function BnSegmented<T extends string>({
  value,
  options,
  onChange,
  testID
}: BnSegmentedProps<T>): JSX.Element {
  return (
