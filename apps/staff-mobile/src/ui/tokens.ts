/**
 * BossNyumba DNA design tokens for staff-mobile — RN-native rebrand
 * of the BossNyumba Tailwind tokens for direct use in StyleSheet.
 *
 * Pulls from the existing app palette (`src/theme/colors.ts`) so colour
 * truth is single-sourced. Adds the BossNyumba gold-on-navy gradients,
 * type ramp, radii, and shadow recipes that the web BossNyumba shell uses,
...
export const tokens = {
  color: {
    // Background ladder (BossNyumba "Midnight Slate")
    ...
    // Signal — warm gold (BossNyumba hero gold)
    gold: colors.gold,
    ...
    // Borders + dividers — BossNyumba uses `rgba(white, .08–.12)` hairlines
  ...
  radius: {
    ...
    md: 16,
    lg: 20,
    xl: 24,    // BossNyumba card 24px (rounded-3xl)
    pill: 999
  },
  ...
  shadow: {
    // Soft drop shadow — BossNyumba signature warm-elevated card.
    card: {
      shadowColor: '#000',
      shadowOpacity: 0.35,
      shadowRadius: 18,
...
export type BnTokens = typeof tokens
