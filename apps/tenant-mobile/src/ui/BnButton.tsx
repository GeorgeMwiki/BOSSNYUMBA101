...
export type BnButtonVariant = 'primary' | 'secondary' | 'ghost' | 'danger'
export type BnButtonSize = 'sm' | 'md' | 'lg'

export interface BnButtonProps {
  readonly label: string
  readonly onPress: () => void
  readonly variant?: BnButtonVariant
  readonly size?: BnButtonSize
...
/**
 * BossNyumba button primitive. Pill-shaped, three variants:
 *  - primary  : warm gold fill + navy text (BossNyumba hero CTA)
 *  - secondary: navy fill + gold outline + gold text
 *  - ghost    : transparent + cream text + 1px navy hairline
 *  - danger   : warm-red fill + cream text
 */
export function BnButton({
  label,
  onPress,
  variant = 'primary',
  size = 'md',
...
const sizeStyles: Record<BnButtonSize, { padX: number; padY: number; minH: number; font: number }> = {
