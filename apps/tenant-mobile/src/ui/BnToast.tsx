...
export type BnToastTone = 'success' | 'warning' | 'critical' | 'info'

export interface BnToastProps {
  readonly tone: BnToastTone
...
/**
 * BossNyumba toast — pill with left dot + body 14/600. Tones map to
 * success / warning / critical / info. Fades in over 200ms,
 * stays until parent flips visible to false.
 */
export function BnToast({ tone, message, visible, testID }: BnToastProps): JSX.Element | null {
...
const toneStyles: Record<BnToastTone, { bg: string; border: string; fg: string; dot: string }> = {
  success: {
