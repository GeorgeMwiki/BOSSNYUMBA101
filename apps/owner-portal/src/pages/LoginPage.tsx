...
export function LoginPage() {
  ...
  const onSubmitMfaVerify = submitVerify(async ({ code }) => {
    ...
    try {
      ...
      setServerError(err instanceof Error ? err.message : t('verificationFailed'));
    }
  });
  ...
  const onSubmitMfaSetup = submitSetup(async ({ code }) => {
    ...
    try {
      ...
      setServerError(err instanceof Error ? err.message : t('mfaSetupFailed'));
    }
  });
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-blue-600 to-blue-800 p-12 flex-col justify-between">
        ...
        <div className="text-blue-200 text-sm">{t('brandFooter')}</div>
      </div>
          ...
          {step === 'credentials' && (
            <>
              ...
              <p className="mt-4 text-center text-sm text-gray-500">
                {t('noAccount')} <Link to="/register" className="text-blue-600 font-medium hover:underline">{t('register')}</Link>
              </p>
            </>
          )}
          ...
          {step === 'mfa_verify' && (
            <>
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-gray-900">{t('mfaVerifyTitle')}</h2>
                  <p className="text-gray-500 text-sm">{t('mfaVerifySubtitle')}</p>
                </div>
              </div>
              ...
              <form onSubmit={onSubmitMfaVerify} className="space-y-4" noValidate>
                <div>
                  <label htmlFor="mfaCode" className="block text-sm font-medium text-gray-700 mb-1">{t('mfaCodeLabel')}</label>
                  <input
                    id="mfaCode"
                    type="text"
                    inputMode="numeric"
            ...
            <>
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-sm">{t('mfaSetupSubtitle')}</p>
                </div>
              </div>
                ...
                <div className="p-4 bg-gray-50 rounded-lg">
                  ...
                  {mfaQrCode ? (
                    <div className="flex justify-center p-4 bg-white rounded-lg border">
                      <img src={mfaQrCode} alt={t('qrAlt')} className="w-48 h-48" />
                    </div>
                  ) : (
                    <div className="flex justify-center p-4 bg-white rounded-lg border">
                      <div className="w-48 h-48 bg-gray-200 rounded animate-pulse flex items-center justify-center">
                        <span className="text-gray-500 text-sm">{t('loadingQr')}</span>
                      </div>
                    </div>
                  )}
                  {mfaSecret && (
