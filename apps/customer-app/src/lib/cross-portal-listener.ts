/**
 * Cross-portal listener (Customer App) — Central Command Phase A C6.
 *
...
export interface CrossPortalListenerOptions {
   ...
   *
   * Closes round-3 H-3 (HIGH): pass a `getToken` callback so every
   * reconnect reads the latest bearer. The static `token` form is
   * still accepted; when both are provided `getToken` wins.
   */
