...
| Agent | Env var (when adapter lands) | Today's behaviour |
|---|---|---|
| Monthly Close — Reconciliation / Statement / Disbursement / Notification ports | `DATABASE_URL` (real Drizzle period-bulk adapters under `services/api-gateway/src/services/monthly-close/` activate when DB is reachable) | **WIRED as of commit `0ac239f`.** Reconciliation joins `payments` × `invoices` for the closing window in one round-trip; statements are written as `draft` rows to `owner_statements`; disbursements emit `MonthlyCloseDisbursementProposed` to `event_outbox` for the eventual payouts worker; notifications insert one `notification_dispatch_log` row per (owner, statement) with `status='pending'`. Errors degrade to logged warnings + safe-default returns — orchestrator never crashes. |
| Monthly Close — Statement PDF render | (no env gate yet — render worker lands in follow-up; will likely read `TYPST_BIN` and the existing rendering interface) | Statement rows persist with `degraded_reason: 'no_pdf_renderer'` until the rendering worker lands. The notification adapter still queues entries; only the PDF artefact is missing. |
| Monthly Close — Event publisher | `DATABASE_URL` (event-outbox-backed publisher activates when DB is reachable) | **WIRED.** Publishes `MonthlyCloseCompleted` and `MonthlyCloseDisbursementProposed` through the durable `event_outbox` for downstream subscribers (process-miner, audit, payouts worker). |
...
| Voice Agent — `VoiceBrainPort` | `ANTHROPIC_API_KEY` + `DATABASE_URL` (BrainKernel constructed at composition root flips voice turns to the real 13-step kernel pipeline) | **WIRED as of commit `eb21991`.** When `ANTHROPIC_API_KEY` is set, voice turns round-trip through the central-intelligence kernel (cache → inviolable → tier → memory → cohort → persona → sensor failover → normalize → judge → drift → policy → confidence → provenance). When unset, transparently falls back to the heuristic-language stub (`sw` / `es` / `fr` / `en` — never hardcodes 'en'). |
